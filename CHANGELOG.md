...
### Internal
- `tools/` holds the two offline data-pipeline builders, now in the repo rather than only on one Mac. `build_trail_pack.py` bakes public trail data (OSM, USGS National Digital Trails, NPS, state GIS) into a versioned SQLite region pack, and `build_routing_graph.py` builds the walkable routing graph the route engine searches over. Both exist so no third-party API is called at runtime and attribution travels with the data in the pack. They were untracked, which meant the packs the app ships had no reproducible source in version control. `.gitignore` gained a Python section in the same commit — `__pycache__/` was sitting next to them and would have been committed as compiled bytecode.
