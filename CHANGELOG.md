...
### Internal
- `scripts/test.sh` keeps xcodebuild's stderr separate from stdout and warns when the number of test-result lines disagrees with passed+failed. With the streams merged, an xcodebuild diagnostic landed on the same line as a test result during a parallel run on 2026-09-09 and ate its `passed` suffix, so the script reported 145 for a run of 146 with no failures — a silent undercount that took three rounds to explain. The verdict was always xcodebuild's own `** TEST SUCCEEDED **`; the counts now say so when they cannot be trusted. Verified that the new check flags that exact log.
