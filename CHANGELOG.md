...
- The tip jar carries a real commitment: tip roughly ten dollars in total, in one go or across several tips, and Wockett Pro is granted free when it ships. Keeping that promise is the whole reason `SupporterLedger` exists — see Internal. The threshold is counted in points rather than currency because App Store prices are per-storefront and there is no sound way to convert them on-device, and because two medium tips is $9.98, which would miss a literal $9.99 line by a cent.
