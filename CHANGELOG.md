...
- Routes results panel content is no longer clipped by the tab bar. The map ignores the safe area, so the panel was laid out over the floating tab bar rather than above it, cutting off the last card instead of letting it scroll clear. The panel now pads its content by the real bottom safe-area inset.
- The Routes tab's *config* panel — the screen you land on before searching — was clipped by the tab bar too. The safe-area fix was applied to only one branch of the `safeAreaInset`, leaving the "Search a place / Nearby places" row under the bar. Worse there than in the results panel, because the config panel has no scroll view, so there was no way for the user to reach the cut-off row. Found by an independent code review pass, not by a test.
- `testRoutesReachable` can now actually fail. It scrolled the results panel until Start Walk was hittable and then asserted hittability — but Start Walk has content below it (Open in Apple Maps, then the community section), so a swipe could always lift it clear of the tab bar regardless of whether the panel was clipped. It passed green while the config panel next door was visibly cut off. The test now makes two assertions that can genuinely fail: the config panel's bottom row must be hittable on arrival (that panel has no scroll view, so a clipped row is permanently unreachable), and the results panel's *last* element must be reachable after scrolling (nothing renders below it, so a clipped panel runs out of content before it clears the bar). New identifiers `routes.nearbyPlaces` and `routes.communityToggle` support this.
...
- The Routes smoke test is now deterministic and offline: under `-WKTUITest` route generation returns a fixed stub instead of calling MKDirections, and the community-routes fetch is skipped, so the test can't fail because a CI runner had flaky network. It also scrolls the results panel before checking that Start Walk is tappable — the panel is capped at 35% of screen height, so the button legitimately renders below the fold, and without scrolling the check could not tell that apart from the button being covered by the tab bar, which is the regression it exists to catch. All five UI smoke tests now pass locally.
